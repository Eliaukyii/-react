...
import ExpenseListFilter from "./component/ExpenseListFilter";
import moment from "moment";
import { ActType, SearchType, StatusType, CommonConfig, DeclareTabData, ApprovalTabData } from '../config/commonConfig';
import LoadMore from "../commonComp/LoadMore";
import ExpenseChooseList from "./component/ExpenseChooseList";
...
import { getDept, getEmployee } from "../commonInterface/DeclareComm";
import { PreDeptFilter } from "../commonComp/EditCommonFilter";
import BaseComm from "../commonInterface/BaseComm";

@observer
class PayExpenseList extends React.Component {
    @observable data = {
        userid: '',
        type: 'ListApproval',
        params: {
    ...
    componentDidMount() {
        ...
        this.data.resData = sessionStorage.getItem("payExpenseList");
        if (this.data.resData !== null && this.data.resData !== undefined) {
            this.data.payExpenseList = JSON.parse(this.data.resData);
    ...
    handleClickAdd = () => {
        window.location.href = '/home/payExpenseAdd?' + BaseComm.UrlVersion();
    };
    ...
    payExpenseList = () => {
        if (!this.data.isShowFilter && !this.data.payExpenseList) {
            this.data.isShowFilter = true;
            this.data.tipsText = '请稍等...';
            this.data.isShowTips = true;
        }
        let self = this.data;
        let data = self.payExpenseList;
        self.noData = false;
        ...
        axios({
            ...
            if (data.length > 0) {
                if (res.data.length > 0) {
                    self.payExpenseList = self.payExpenseList.concat(res.data);
                ...
                if (res.data.length > 0) {
                    ...
                    if (res.data.length < 10) {
                        self.isShowLoadMore = true;
                        self.moreText = CommonConfig.loadFinishTxt;
                        sessionStorage.setItem("payExpenseList", JSON.stringify(self.payExpenseList));
                    } else {
                        self.isShowLoadMore = true;
                        self.moreText = CommonConfig.loadMoreTxt;
                        sessionStorage.setItem("payExpenseList", JSON.stringify(self.payExpenseList));
        ...
        return (
            <div className="wrapper">
                <div className="wrapper-detail">
                    ...
                    <HeadTitle titleTxt={this.data.title} showAddBtn={this.data.title === '报销申报'}
                    ...
                    <div className="list-content" ref={content => { this.content = content }}>
                        {this.data.payExpenseList.map((item, index) => (
