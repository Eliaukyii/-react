...
import { getAttachment, getFileDownload, getFilePreview, getProcess } from "../commonInterface/DeclareComm";
...
@observer
class PayBorrowAudit extends React.Component {
    @observable data = {
        type: '',
        userid: '',
        params: {
    ...
    ConfirmAuditReal = (signImg) => {
        ...
        if (this.data.auditType === 'audit') {
            ...
            axios.post(URL.histBorrow, params).then(res => {
                this.data.isShowTips = false;
                this.data.tipsText = '';
                if (res.status === 200) {
                    ...
                    <div className="content_mainS" style={{ display: this.data.isShow ? 'block' : 'none' }}>
                        <div className="content_mainDetail">
                            <div className='detailBorder borderBottom'>
                                ...
                                <DetailBold label="借支部门" value={detail.Unit} />
                                ...
                                <DetailBold label="借支人" value={detail.Anyperson} />
                                <DetailBold label="申报人" value={detail.Applicant} />
                            </div>
                            <DetailNormal className='detailMarTop' label="项目编号" value={detail.ItemID} />
                            <DetailNormal label="借支事由" value={detail.Anyreason} />
                        </div>
