...
@observer
class PayAllowanceAudit extends React.Component {
    @observable data = {
        type: '',
        userid: '',
        params: {
    ...
    getChecker = (PI_ID, PFI_ID) => {
        axios({
            ...
            if (res.data[0].To !== undefined) {
                this.data.isBtnHide = false;
                this.data.params.To = res.data[0].To;
                this.data.params.ToValue = res.data[0].ToValue;
            } else {
    ...
    ConfirmAuditReal = (signImg) => {
        ...
        if (this.data.auditType === 'audit') {
            ...
            axios.post(URL.histAllowance, params).then(res => {
                this.data.isShowTips = false;
                this.data.tipsText = '';
                if (res.status === 200) {
                    ...
                    <div className="content_mainS" style={{ display: this.data.isShow ? 'block' : 'none' }}>
                        <div className="content_mainDetail">
                            <div className='detailBorder borderBottom'>
                                ...
                                <DetailBold label="领款部门" value={detail.Unit} />
                                <DetailBold label="经办人" value={detail.Attn} />
                                <DetailBold label="领款人" value={detail.Anyperson} />
                            ...
                            <DetailNormal className='detailMarTop' label="领款事由" value={detail.Anyreason} />
                            <DetailNormal label="经费用途" value={detail.use} />
                            <DetailNormal label="挂账科目" value={detail.PaymentCourse} />
                            <DetailNormal label="科目全称" value={detail.AllCourse} />
                        </div>
                        <div className="spaceG" />
                        <div className='content_mainDetail'>
                            <DetailBold label="总金额" value={`${detail.Money_1 || '0.00'}元`} />
...
export default PayAllowanceAudit;